...
lin: $(OUTPUT)/lin
$(OUTPUT)/lin: portacle_lin.c portacle.c fontreg_lin.c fontreg.c ld-wrap.c Makefile
	$(CC) -o "$(OUTPUT)/ld-wrap.so" $(CFLAGS) -D_GNU_SOURCE -fPIC -shared -Wl,-init,init "ld-wrap.c" -ldl
	$(CC) -o "$(OUTPUT)/fontreg" $(CFLAGS) -static -D_GNU_SOURCE "fontreg.c"
...
mac: $(OUTPUT)/mac
$(OUTPUT)/mac: portacle_mac.c portacle.c fontreg_lin.c fontreg.c Makefile
